...
> **HLX는 계획을 고정하지만, Step은 실행 파라미터를 고정한다.**
>
> 이것이 멱등성의 이중 확보이다. HLX가 "무엇을 할 것인가"의 흔들림을 막고, Step이 "어떻게 할 것인가"의 흔들림을 막는다. 두 계층이 함께 작동하여, LLM의 비결정성이 실제 실행에 미치는 영향을 구조적으로 최소화한다.
